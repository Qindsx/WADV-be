import { Column, Entity, PrimaryGeneratedColumn } from 'typeorm';

@Entity('major_agricultural_products_6', { schema: 'agricultural_statistics' })
export class MajorAgriculturalProducts_6 {
  @PrimaryGeneratedColumn({ type: 'int', name: 'id' })
  id: number;

  @Column('int', { name: 'grains', nullable: true, comment: '粮食' })
  grains: number | null;

  @Column('int', { name: 'cotton', nullable: true, comment: '棉花' })
  cotton: number | null;

  @Column('int', { name: 'oil_bearing', nullable: true, comment: '油料' })
  oilBearing: number | null;

  @Column('int', { name: 'vagetables', nullable: true, comment: '蔬菜' })
  vagetables: number | null;

  @Column('int', { name: 'fruit', nullable: true, comment: '水果' })
  fruit: number | null;

  @Column('int', {
    name: 'slaughtered_hogs',
    nullable: true,
    comment: '生猪出栏',
  })
  slaughteredHogs: number | null;

  @Column('int', {
    name: 'slaughtered_poultry',
    nullable: true,
    comment: '家禽出笼',
  })
  slaughteredPoultry: number | null;

  @Column('int', { name: 'eggs', nullable: true, comment: '禽蛋产量' })
  eggs: number | null;

  @Column('int', { name: 'milk', nullable: true, comment: '牛奶' })
  milk: number | null;

  @Column('int', {
    name: 'aquatic_products',
    nullable: true,
    comment: '水产品',
  })
  aquaticProducts: number | null;

  @Column('varchar', {
    name: 'year',
    nullable: true,
    comment: '年份',
    length: 255,
  })
  year: string | null;

  toJSON() {
    return {
      grains: this.grains,
      cotton: this.cotton,
      oilBearing: this.oilBearing,
      vagetables: this.vagetables,
      fruit: this.fruit,
      slaughteredHogs: this.slaughteredHogs,
      slaughteredPoultry: this.slaughteredPoultry,
      eggs: this.eggs,
      milk: this.milk,
      aquaticProducts: this.aquaticProducts,
      year: this.year,
    };
  }
}
